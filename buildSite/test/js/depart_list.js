(function(){
	$(function(){
		localStorage.unloginUrl = "depart_list.html";
		/*阻止默认事件*/
		...
		var sHtml = '',
			sPrevName = localStorage.getItem("depart_guide_two_name"),
			iPrevId = localStorage.getItem("depart_guide_two_id"),//医院科室编号
			...
			success: function(data){
				if(data.code ==1){
					...
					if(iLen){
						//添加数据
						for (i=0; i<iLen; i++) {
							sHtml += '<div class="sectionlist clearfix" hospId="'+data.results[i].hospId+'" ><div class="sectionlist-l fl clearfix"><div class="sectionImg fl"><img src="'+ data.results[i].hospImage +'" alt="" /></div><div class="sectionDetail fl"><p class="sectionName fs16"><span class="colortwo sectionname">'+ data.results[i].hospName +'</span></p><p class="section"><span class="level radius3 fs12 textcenter">'+ data.results[i].hospGraded +'</span></p></div></div><div class="sectionlist-r fr textright"><span class="font-icon textcenter radius5 fs12">n</span></div></div>';
						}
						$('.scroll').append(sHtml);
						$('#loading').remove();
						scroll.refresh();
