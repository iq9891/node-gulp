(function(){
	$(function(){
		...
		//console.log(localStorage.getItem("disease_guide_id"));
		$.ajax({
			type: 'GET',
			url: config.Rurl+'api/v1/guide/disease/getByBodyId?bodyId='+localStorage.getItem("disease_guide_id"),
			dataType:'json',
			...
			success: function(data){
				if(data.code ==1){
					...
					//添加数据
					for (i=0; i<iLen; i++) {
						sHtml += ' <div class="depart" _href="dis_pa_guide.html" id='+data.results[i].id+'>'+ data.results[i].cont +'<span class="enterdepart">n</span></div>';
					}
		...
		$('.depart').live('tap', function(){
			localStorage.setItem("dis_pa_guide", $(this).attr('id'));
